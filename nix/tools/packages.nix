...
let
  ...
  optionalPackage = name: package:
    lib.optionalAttrs (supports name) { "${name}" = package; };
in
  (optionalPackage "summarize" (pkgs.callPackage ./pkgs/summarize.nix {
